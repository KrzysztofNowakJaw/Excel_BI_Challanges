...
names <- read_xlsx("PQ_Challenge_127.xlsx", range = "A1:B11") |>
  janitor::clean_names()

# Read data from Excel file and clean column names for 'Zodiak'
Zodiak <- read_xlsx("PQ_Challenge_127.xlsx", range = "D1:D13") |>
  janitor::clean_names()

# Process 'Names' data: create 'MonthDay' column and convert it to Date format
Names <- names |>
  mutate(
    MonthDay = paste("1990", format(date_of_birth, "%B %d"), sep = "-"),
...
ZodiakCleaned <- Zodiak |>
  ...
  mutate(
    across(!contains("Sign"), \(x) paste("1990", x, sep = "-")),
