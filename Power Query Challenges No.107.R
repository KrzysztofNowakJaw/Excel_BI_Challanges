...
Combined$Category <- factor(Combined$Category,levels = c("Manager","Expert"),labels = c(1,2),ordered = TRUE)
...
Answer <- Combined |>
  ...
  mutate(`Question ID` = case_when(is.na(`Question ID`) ~ paste('Q',as.character(QNR),sep = "-"),.default = `Question ID`)) |>
